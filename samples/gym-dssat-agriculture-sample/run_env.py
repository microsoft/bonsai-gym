...
def interact_with_env(env, verbose=True):
    ...
    while not env.done:
        ...
        if verbose:
            ...
            pprint(
                f'dap : {dap} -> fertilizing {action["anfer"]} kg N/ha ; reward {reward}'
            )
...
def multiprocess_trial_hard_reset(env, cwd, rep, save_log=False):
    ...
    with multiprocessing.Pool() as pool:
        raw_result = list(
            pool.imap_unordered(_multiprocess_trial_func_hard_reset, arguments)
        )
    return raw_result
...
if __name__ == "__main__":
    ...
    for i, mode in enumerate(["all", "fertilization", "irrigation"]):
        print(f"MODE: {mode}")
        env_args = {
        ...
        if try_interact:
            try:
                ...
                if mode == "mode":
                    env.render(
                        type="ts", feature_name_1="cleach", feature_name_2="totaml"
                    )
                    env.render(type="reward", cumsum=True)
                    env.render(type="reward", cumsum=False)
                env.reset_hard()
            except Exception as e:
                logging.exception(e)
            finally:
