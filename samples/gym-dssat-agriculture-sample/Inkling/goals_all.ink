    ...
    concept Fertilize(input): Action {
        curriculum {
            ...
            goal (state: SimState, action: Action) {
                maximize Yield : state.grnwt  in Goal.RangeAbove(5000)
                # avoid deadCrops: state._gym_terminal in Goal.RangeAbove(1)
                minimize Runoff: action.anfer in Goal.RangeBelow(30)
            }
            training {
                NoProgressIterationLimit: 10 * 1000 * 1000,
                EpisodeIterationLimit: 159
              }
